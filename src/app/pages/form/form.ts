...
export class Form implements OnInit {
  ...
  validateCurrentStep(): boolean {
    ...
    if (this.currentStep === 1) {
      if (!this.HVACType) {
        this.errors['HVACType'] = 'Please select a service.';
        valid = false;
      }
      ...
      if (!this.BriefRequirement.trim()) {
        this.errors['BriefRequirement'] = 'Please provide a brief description of your HVAC needs.';
        valid = false;
      }
  ...
  async submit() {
    ...
    if (this.validateCurrentStep()) {
      ...
      const payload = {
        HVACType: parseInt(this.HVACType),
      ...
      this.http.post('https://hvacinus.com/server/forward-lead', payload).subscribe({
        next: (response) => {
    ...
    try {
      ...
      const poll = () => {
        ...
        if (val) {
          ...
          const pollInterval = this.aff_id || this.transaction_id || this.sub_aff_id ? 2000 : 300;
          this.leadiDPollTimer = setTimeout(poll, pollInterval);
        }
      };
      const delay = this.aff_id || this.transaction_id || this.sub_aff_id ? 10000 : 500;
